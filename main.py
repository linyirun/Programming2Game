...
def main():
        ...
        # Stage lasts until 40 seconds
            ...
            if ticks % 40 == 0:
                ...
                for i in range(random.randrange(5, 8)):
                    ...
                    all_sprites.add(obstacle)
                    obstacle_sprites.add(obstacle)
        # Stage 3: Hard - 7 - 10 obstacles per wave, waves every 30 ticks, each obstacle varies in speed from 2 - 10
        # TODO: add a laser every 40 ticks - blue-ish warning, after 1 second then activate
        elif ticks < 90 * TPS:
            if ticks % 30 == 0:
    ...
    while True:
        ...
        write_text(f"You survived for {round(ticks / TPS - 5, 3)} seconds.", 50, 100, 30)
        pygame.display.flip()
        tick += 1
        if tick == 5 * TPS:
            break
